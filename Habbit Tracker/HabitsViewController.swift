...
class HabitsViewController: UIViewController, HabitsModelUser, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        self.selectedHabit = self.habitsModel.habits[indexPath.row]
        performSegue(withIdentifier: "Edit habit", sender: self)
    }
        
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if var destination = segue.destination as? HabitsModelUser {
            destination.habitsModel = self.habitsModel
            destination.selectedHabit = self.selectedHabit
        }
    }
}
